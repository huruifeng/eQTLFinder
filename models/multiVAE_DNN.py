...
def multiVAE_DNN(dataset_X,dataset_Y):
    ...
    output_model_file = os.path.join('', 'Roadmap_VAE_DNN_architecture_' + st + '.png')
    ...
    output_model_file = os.path.join('', 'TF_VAE_DNN_architecture_' + st + '.png')
    ...
    x = Input(shape=(original_dim,),name="mVAE_DNN_DNAacc_Input")
    h = Dense(intermediate_dim, activation='relu',name="mVAE_DNN_DNAacc_hidden")(x)
    z_mean = Dense(latent_dim,name="mVAE_DNN_DNAacc_zmean")(h)
    z_log_sigma = Dense(latent_dim,name="mVAE_DNN_DNAacc_zlogsigma")(h)

    z = Lambda(sampling, output_shape=(latent_dim,),name="mVAE_DNN_DNAacc_z")([z_mean, z_log_sigma])

    h_decoded = Dense(intermediate_dim, activation='relu',name="mVAE_DNN_DNAacc_hdecode")(z)
    x_decoded_mean = Dense(original_dim, activation='sigmoid',name="mVAE_DNN_DNAacc_decodemean")(h_decoded)
    ...
    output_model_file = os.path.join('', 'DNAacc_VAE_DNN_architecture_' + st + '.png')
    ...
    output_model_file = os.path.join('', 'mVAE_DNN_architecture_' + st + '.png')
    ...
    y_pred_x = model.predict(np.array(test_X))
    y_pred = y_pred_x > 0.5
